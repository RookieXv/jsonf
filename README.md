...
JSONF 是一个本地优先的开发者工具平台，基于 Vue 3、Vue Router、TypeScript 和 Vite 构建。它不只是一个简单的格式化按钮，而是把 JSON 格式化、校验、修复、搜索、树形查看和节点编辑放在同一个高效界面里，适合日常排查接口日志、整理配置文件、查看复杂响应体和准备可读的 JSON 片段。
