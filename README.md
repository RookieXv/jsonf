...
- JSON 解析、格式化和转换逻辑优先放到 `src/tools/json/services/`。
- 工具私有状态与组件放到对应的 `src/tools/<tool>/` 目录，页面级入口放到 `src/pages/`。
